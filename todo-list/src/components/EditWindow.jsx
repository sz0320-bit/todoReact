...
const EditWindow = ({ onSubmit , showState, initial}) => {
    ...
    return (
        <div className={"h-[100%] left-0 top-0 justify-center  items-center w-screen fixed flex-col flex glass "}>
            <form onSubmit={newTask} className={'flex font-mono primary flex-col center w-[90%] h-[25em] shadow-2xl  justify-evenly items-center  rounded-xl p-2 '}>
                <h2 >Please Enter Task:</h2>
                <textarea id="mainentry" value={text}  className={'font-mono h-[60%] text-top text-xl border-2 border-blue-800 bg-[rgba(0,0,0,0.85)] text-white rounded-xl p-3  w-[90%]'} ref={ref} onChange={(e) => setText(e.target.value)}/>
