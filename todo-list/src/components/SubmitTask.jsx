...
const SubmitTask = ({ onSubmit , showState}) => {
    ...
    return (
        ...
        <form onSubmit={newTask} className={'flex font-mono  gap-5 flex-col center w-[90%] py-5 h-fit shadow-2xl  justify-evenly items-center primary rounded-xl p-2 '}>
            <h2 className={"text-white"}>Please Enter Task:</h2>
            <textarea id="mainentry"  className={'font-mono h-[60%] text-top text-white text-xl bg-black border-2 border-blue-800 rounded-xl p-3  w-[90%]'} ref={ref} onChange={(e) => setText(e.target.value)}/>
