    ...
    const Window = ({ onSubmit , showState, initialTitle, initialDesc,initialTime,title}) => {
        ...
        return (
            <div className={"h-[100%] left-0 top-0 justify-center  items-center w-screen fixed flex-col flex glass "}>
                <form onSubmit={newTask} className={'flex font-mono primary flex-col center w-[90%] h-fit gap-5 py-7 shadow-2xl  justify-evenly items-center  rounded-xl p-2 '}>
                    ...
                    <input type={"text"} id="mainentry" maxLength={24} value={text}  required={true}  className={'font-mono bg-transparent h-14 text-top text-xl border-2 overflow-auto border-blue-800  textColor rounded-xl p-3  w-[90%]'} ref={ref} onChange={(e) => setText(e.target.value)}/>
