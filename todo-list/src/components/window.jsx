                ...
                <motion.div
                    ...
                    <form onSubmit={newTask}
                        ...
                        <input type={"text"} required id="mainentry" maxLength={24} value={text}
                               className={'font-mono bg-transparent h-14 text-top text-xl border-2 overflow-auto border-blue-800  textColor rounded-xl p-3  w-[90%]'}
                               ref={ref} onChange={(e) => setText(e.target.value)}/>
                        <h4 className={"textColor mb-[-1em] text-[0.8rem]"}>Description:</h4>
                        <textarea id="mainentry" value={description}
                                  className={'font-mono bg-transparent h-40 text-top text-xl border-2 overflow-auto border-blue-800  textColor rounded-xl p-3  w-[90%]'}
                                  ref={ref} onChange={(e) => setDescription(e.target.value)}/>
                        <div className={"flex flex-row gap-16"}>
                            <input type="button" onClick={cancelText} value="cancel"
                                   className={'px-7 font-mono py-0.5 bg-blue-600 w-32 text-white text-center  h-fit px-2 rounded-xl shadow-xl'}
                                   id="mainsubmit"/>
                            <input type="submit" value="save"
                                   className={'px-7 font-mono py-0.5 bg-blue-600 w-32 text-white  h-fit px-2 rounded-xl shadow-xl'}
                                   id="mainsubmit"/>
                        </div>
                    </form>
                </motion.div>
